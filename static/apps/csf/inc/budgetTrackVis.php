...
<?php
	...
	foreach($currentUser->order as $purchase){
		...
		$dailySpent[$relDay] += $purchase->money;
	}

	// upperBound is used in determining the # of measures along the y-axis.	
	$upperBound = max(max($dailySpent),$currentUser->budget['money']/30);
...
<!--Y-AXIS LABEL-->
	<text x="20" y="210" fill="black" style="font-size:14pt;font-weight:bold;" transform="rotate(-90 30,210)">DAILY SPENDING</text>
<!--Y-AXIS MARKERS-->

<?php
	$nSegments = (int)($upperBound / 10) + 1; //number of measures
	$vSegmentSize = 10;  // the size of the increment
	...
	<text x="40" y="'.(305 - $i*$vspace).'" fill="black" style="font-size:8pt;">'.($i*$vSegmentSize).'</text>');
?>
