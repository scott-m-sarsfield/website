...
import sherlockReducer, {
  initialState,
  actions,
  stages,
} from './sherlock_reducer';
...
const SherlockGame = () => {
  ...
  switch (stage) {
      ...
      content =
        gameType === 'cards' ? (
          <CardGuessLineupContent
            ...
            onResponse={(numberDisplayed) =>
              dispatch({ type: actions.ELIMINATE, numberDisplayed })
            }
            animated={animated}
          />
        ) : (
    ...
    default:
      content = (
        <IntroContent
          onChooseGameType={(gameType) =>
            dispatch({ type: actions.CHOOSE_GAME_TYPE, gameType })
          }
          animated={animated}
        />
      );
  ...
  return (
    <SherlockFrame
      animated={animated}
      onToggleAnimation={() => dispatch({ type: actions.TOGGLE_ANIMATION })}
    >
      {content}
    </SherlockFrame>
  );
